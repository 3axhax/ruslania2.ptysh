...
}(function ($, undefined)
{
    ...
    $.widget('mp.marcoPolo', {
        ...
        _showList: function ()
        {
            ...
            {
                $list.css({ left: this.$input.position().left});
                $list.show();
            }
...
//            return this;
        ...
        _buildResultsList: function (q, data)
        {
            ...
            for (var i = 0, length = data.length; i < length; i++)
            {
                ...
                formatItem = options.formatItem.call($input, datum, $item, q);
        ...
        _request: function (q)
        {
            ...
            self.timer = setTimeout(function ()
            {
                ...
                $.each(self.options.dynamicData, function (idx, val)
                {
                    if (val != null && val != undefined)
                    {
                        var v = ($.isFunction(val)) ? val() : val;
                        if (v != undefined && v != null) params[idx] = v;
                    }
                });
