...
class DGSphinxSearch extends CApplicationComponent
{
     ...
     * @return array
     */
    public function query($query, $index='*', $comment='', $resetAll=true)
    {
    return $this->doSearch($index, $query, $comment, $resetAll);
    }

    public function EscapeString($q)
    {
        return $this->client->EscapeString($q);
    }
    ...
    public function filters($filters)
    {
        ...
        if ($this->enableResultTrace) {
            Yii::trace("Filters: " . print_r($filters, true), 'CEXT.DGSphinxSearch.doSearch');
        }
     ...
     * @return array
     */
    protected function doSearch($index, $query = '', $comment = '', $resetAll = true)
    {
        if (!$index) {
        ...
        if ($this->enableResultTrace)
        {
            ...
            Yii::trace("With filters: " . print_r($this->client->_filters, true), 'CEXT.DGSphinxSearch.doSearch');
        }
        ...
        if ($this->enableResultTrace) {
            Yii::trace("Total found: " . $res['total_found']."\n--------------------------------------\n", 'CEXT.DGSphinxSearch.doSearch');
        }
        ...
        if($resetAll) $this->resetCriteria();
        return $res;
    }
